...
def build_cnn_model(
    ...
    """
    建立端到端深層 CNN，設計用於攻擊帶有 Masking 保護的 AES 實作。

    架構（對應論文 Figure 3）:
        4 個漸進式 Conv Block（filters 從 32 增至 256，kernel 從大到小）
        → Flatten → Dense(256) → Dropout → Softmax(256)

    每個 Block 的大 MaxPooling（stride=4）快速壓縮時間軸，
    幫助 CNN 在長 trace 中找到關鍵洩漏點。
    """
    inputs = layers.Input(shape=input_shape)

    # 4 個 Conv Block：filters 倍增，kernel 縮小
    x = _conv_block(inputs, filters=32,  kernel_size=11)
    x = _conv_block(x,      filters=64,  kernel_size=7)
    x = _conv_block(x,      filters=128, kernel_size=5)
    x = _conv_block(x,      filters=256, kernel_size=3)
    ...
    model = models.Model(inputs=inputs, outputs=outputs, name="ASCADv2_CNN_Mixed")
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=2e-4, 
                                            beta_1=0.9,      # <--- 這裡就是 β1 (也等同於傳統的 β 動量因子)
                                            beta_2=0.999,    # <--- 這裡就是 β2
                                            epsilon=1e-7     # <--- 這裡就是 ε
                                        ),
        loss="categorical_crossentropy", # 交叉墒
...
def main() -> None:
    ...
    config = {
        # 資料
        "profiling_limit": 10000,   # 建議: GPU 夠強可拉高至 30000 提升破譯率
        "attack_limit": 2000,
        "val_ratio": 0.1,
        # 訓練
        "batch_size": 128, # 樣本數
